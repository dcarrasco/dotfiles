...
if [ "$DESKTOP_SESSION" == "bspwm" ]; then
    ...
    keybinds=$(
        ...
        sed 's/^0//' |
        sed 's/^8/[ALT]+/' |
        sed 's/mouse:272/[mouse left click]/' |
        sed 's/mouse:273/[mouse right click]/' |
        awk -F, 'function rep(n,c){s=""; for(i=0;i<n;i++) s=s c; return s;} {print $1 $2 rep(30-length($1)-length($2), " ") $3}'
    )
fi

theme_str="* {font: \"JetBrainsMono Nerd Font 12\";} window{width:40%; height:70%;}"
echo "$keybinds" | rofi -dmenu -i -p "Keybinds" -theme-str "$theme_str"
